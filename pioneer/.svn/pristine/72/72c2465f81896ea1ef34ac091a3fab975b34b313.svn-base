...
ImageTrans.prototype = {
  ...
  load: function(src) {
	if ( this._support ) {
		...
		img.ondblclick=function(){window.open(src)}
		this.onPreLoad();
		img.src = src;
	}
  },
	...
	return {
		css3: {//css3设置
			...
			load: function(){
				...
				$$D.setStyle( img, {//居中
					top: ( this._clientHeight - imgheight ) / 2 + "px",
					left: ( this._clientWidth - imgwidth ) / 2 + "px",
					visibility: "visible"
				});
			},
			show: function() {
		...
		canvas: {//canvas设置
			...
			show: function(){
				...
				context.drawImage( img, -imgwidth / 2, -imgheight / 2 );//居中画图
				context.restore();
			},
			dispose: function(){
