...
<script type="text/javascript">
    page.onLoad = function() {
                page.send({
                    url : contextPath + "/tSysGroup/getBeanById/?${pageContext.request.queryString}"
                }).then(function(data) {
                   ...
                   if(pid==0){
                	   $('#indexTree').combotree('setValues', '');
                   }else{
                	   $('#indexTree').combotree('setValues', pid);
                   }
                	
                });

                //page.loadForm({url : contextPath + "/tSysGroup/getBeanById/?${pageContext.request.queryString}"});
	...
	$(document).ready(function(){
		  ...
		  /*  $('#pid').combobox({    
		   ...
		   $('#indexTree').combotree({
				url : contextPath + "/tSysGroup/getGroupTree?groupType="+${param.groupType},
	...
	<jsp:body>
	<form method="post" action="${ contextPath }/tSysGroup/edit">
        ...
        <table class="default_form_input">
            ...
            <tr id="groupType_tr">
                ...
                <td>
                            ...
                            <input id="groupType" name="groupType"  type="text"
                                      class="easyui-validatebox textbox"
                                   data-options="" />
