...
<script type="text/javascript">
    page.onLoad = function() {
                page.send({
                    url : contextPath + "/tSysBreed/getBeanById/?${pageContext.request.queryString}"
                }).then(function(data) {
                    //  item = kernel.json.parse(data);
                    page.loadForm({data:data });
                });

                //page.loadForm({url : contextPath + "/tSysBreed/getBeanById/?${pageContext.request.queryString}"});
	...
	<jsp:body>
	<form method="post" action="${ contextPath }/tSysBreed/edit">
