...
(function() {
	...
	page.saveForm = function(formId, processId, fid, checkType) {
		...
		$("#processId").remove();
		$(form).append('<input type="hidden" id="processId" name="processId" value="'+processId+'">');
		...
		var opt = {
			success : function(data) {
				if (new RegExp("^(\\{|\\[)").test(data)) {
					...
					if (data.status == 1) {
						...
						setTimeout(function(){$(window.parent.document).find("#myModal4").modal('hide')},1000);
						if(page.url){
							setTimeout(function(){window.location.href = page.url;},2000);
							
						}
						//$(window.document).find("#myModal4").modal('hide');
						//setTimeout(function(){$(window.parent.document).find("#myModal4").modal('show')},1000);
					}
			...
			onSubmit : function() {
				...
				var v = false;
				// 执行验证
				//b true 验证通过, false 验证失败
				validateForm(formId, processId, function(b, noGroupForm){
					v = b;
					if(b){//验证通过
					...
					//关闭套表验证窗口
				   if(!noGroupForm && v){
					   $("#formSubmitBtn").attr("disabled","disabled");//禁用提交按钮
					   $("#formSubmitBtn").css("background", "gray");
					   setTimeout(function(){
						   $(window.parent.document).find("#tbValidate").modal('hide'); 
					   }, 1000);
				   }
				});
				flag++;
...
function validateForm(formId, processId, callback){
	 ...
	 if(warnInfo){
	  ...
	  var tbqyid = $("#tbqyid").val();
	  var tbzq = $("#tbzq").val();
	  
	  //查询逻辑规则验证填的个数，在后台判断是否都已经解释
	  var explainMsgs = [];
	  $(".explainMsg").each(function(){
		  var v = $(this).val();
		  if($.trim(v)){
			  var ruleId = this.id;
			  explainMsgs.push(ruleId);
		  }
	  });
	  
	  var params = "&tbzq=" + tbzq + "&tbqyid=" + tbqyid + "&formId=" + formId + "&processId=" + processId + "&datas=" + JSON.stringify(rowDatas);
	  if(explainMsgs.length > 0){
		  params += "&explainMsgs=" + explainMsgs.join(",");
	  }
	  
	  var hasErr = false;// true 存在错误
	  var noGroupForm = true;// ture 非套表
	  $.ajax({
		   type: "POST",
		   url: url,
		   data: params,
		   ...
		   success: function(msgs){
			   ...
			   var group_form_validate = msgs.group_form_validate;//套标验证
			   var nomsg = '<div class="nomsg"><span class="ico icon-coffee"></span>暂无验证信息！</div>';
			   
			   if(err_form || err_in_form){
				   if(err_form){// 单值规则
					   hasErr = true;
					   ...
					   for(var i = 0; i < err_in_form.length; i++){
						   ...
						   for(var j = 0; j < msgs.length; j++){
							   ...
							   showMsg += '<p id="' +　ruleId　 +　'_t" class="' + className + '" onclick="locationErr(\'' + rowId + '\', this)">' + index + "、";
							   ...
							   if(ruleLevel == "警告"){
								   var rv = $("#" + ruleId).val();
								   if(rv){
									   showMsg += '<p><input type="text" id=' +　ruleId　 +　' value="' + rv + '" readonly="readonly" class="form-control explainMsg" name="content" placeholder="请输入解释信息" onclick="openCommonWord(\''+ruleId+'\')"></p>';
								   }else{
									   // 将规则id作为input的id
									   showMsg += '<p><input type="text" id=' +　ruleId　 +　' readonly="readonly" class="form-control explainMsg" name="content" placeholder="请输入解释信息" onclick="openCommonWord(\''+ruleId+'\')"></p>';
								   }
							   }
				   ...
				   if(group_form_validate){
					   //console.log(group_form_validate)
					   //[{"formName":"全国蛋鸡饲养规模情况","msg":"未暂存数据","status":0},
					   //{"formName":"重庆市畜产品及饲料集市价格周报表","msg":"验证通过","status":1}]
					   $(window.parent.document).find("#tbValidate").modal('show'); 
					   var validateMsg = $(window.parent.document).find("#validateMsg");
					   var html = "<table style='width:100%'>";
					   for(var i = 0; i < group_form_validate.length; i++){
						   var msg = group_form_validate[i];
						   var index = i + 1;
						   if(msg.status == 0){
							   html += "<tr class='tr_error'>"; 
						   }else{
							   html += "<tr>"; 
						   }
						   
						   html += "<td width='35px'>" + index + "、</td>";
						   html += "<td width='360px'>" + msg.formName + "</td>";
						   html += "<td width='160px'>" + msg.msg;+ "</td>";
						   if(msg.status == 0){
							   hasErr = true;
							   html += "<td><span style='color:red;' class='icon-remove'></span></td>";
						   }else{
							   html += "<td><span style='color:green;' class='icon-ok'></span></td>";
						   }
					   ...
					   html += "</table>";
					   if(hasErr){
						   html += "<br><span style='color:red;'>未通过验证，请修改后重试。</span>";
					   }else if(callback){
						   html += "<br><span style='color:green;'>验证通过，正在提交，请稍后...</span>";
					   }else{
						   html += "<br><span style='color:green;'>验证通过。</span>";
					   }
					   
					   validateMsg.html(html);
					   noGroupForm = false;
				   }
			   }
			   ...
			   if(!showMsg){
				   if($(".explainMsg").length = 0){
					   $("#form_check_msg").html(nomsg);
				   }
				   else{
					   var msg = '';
					   $(".explainMsg").each(function(){
						   var t = $("#"+this.id + "_t").prop("outerHTML");
						   msg += t + '<p><input type="text" id=' +　this.id　 +　' value="' + this.value + '" readonly="readonly" class="form-control explainMsg" name="content" placeholder="请输入解释信息" onclick="openCommonWord(\''+this.id+'\')"></p>';
						  });
					   $("#form_check_msg").html(msg);
				   }
				   $("#form_check_msg").show();
			   }
...
//打开常用语选择框
function openCommonWord(ruleId){
	//window.parent.$('#myModalCommonWord').modal('show');
    var sfm = window.document.getElementById("iframeCommWord");
    var expWorld = $("#" + ruleId).val();
    //sfm.src = contextPath+ "/tSysWord/viewToCommWord?ruleId="+ruleId;
    var url = contextPath+ "/pages/form/commonWordList.jsp?ruleId="+ruleId;
    var ht = getWindowHeight();
    if(loginType == 2){
    	url = contextPath+ "/pages/form/commonWordListP.jsp?ruleId="+ruleId;
    	$('#myModalCommonWordW').css("width", getWindowWidth()-20);
    	$('#myModalCommonWordH').css("height", ht - 210);
    }
    
    if($.trim(expWorld)){
    	url += '&expWorld=' + encodeURIComponent(expWorld);
    }
    
    sfm.src = url;
    $('#myModalCommonWord').modal('show');
    
    sfm.onload = function(){
    	if(loginType == 2){
    	    $("#iframeCommWord").contents().find("#selectsPublic").css("height", ht - 350);
    	}
    }
    
   // }else{
  //  	window.location.href=contextPath+ "/pages/form/commonWordListP.jsp?ruleId="+ruleId;
  //  }
    
    //sfm.onload = function(){
    //    window.parent.$('#myModalCommonWord').modal('show');
    //}
    //window.parent.document.getElementById("iframe6").src = contextPath + "/pages/form/loading.html";
    
}

//初始化表单的宽度
function initFormWidth(){
	window.parent.$("#bodybg_box").removeClass("turn_off");
	var wh = getWindowWidth();
	if(wh < 1360){
		window.parent.$("#bodybg_box").addClass("turn_off");
	}else{
		var pd = (wh - 1360)/2;
		$("#bodyMain").css("padding-right", pd + "px");
		$("#bodyMain").css("padding-left", pd + "px");
	}
	
	var th_5 = $(".th_5");//当存在两列输入框时，调大宽度
	if(th_5.length > 0){
		$("#bodyMain").css("padding-right", "0px");
		$("#bodyMain").css("padding-left", "0px");
		if(wh > 1450){
			var pd = (wh - 1450)/2;
			$("#bodyMain").css("padding-right", pd + "px");
			$("#bodyMain").css("padding-left", pd + "px");
