...
var clickName = '';
	...
	var updateNum = function() {
						...
						//初始化分页信息
						initPageMsgNew = function(total) {
							totalCountNew = total;
							pageCountNew = Math.ceil(totalCountNew / pageSizeNew);

							$("#pageSizeNew").html(pageSizeNew);
							$("#totalCountNew").html(totalCountNew);
							$("#pageCountNew").html(pageCountNew);
							$("#pageCount2New").html(pageCountNew);
							$("#nowPageNew").val(nowPageNew);
							$("#nowPage2New").html(nowPageNew);
						}
...
<div class="modal fade" id="loading" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="modal fade" id="myModal2" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true" style="top:1%;margin-left:-700px;" style="z-index:999; ">
		<div class="modal-dialog">
			<div class="modal-content" style="width:1300px">
				<div class="modal-header">
					<button type="button" class="close" onclick="closeModel()"
						aria-hidden="true">×</button>
					<h4 class="modal-title" id="myModalLabel"><span class="icon-comments gd" style="font-size:30px;margin-right:20px;"></span><span class="text"></span></h4>
				</div>

				<div class="gdthrid modal-body tkform gd"   id="div_thirdinfo"></div>
		...
		<tr >
 		...
 		<tr >
			...
			<td>部分恢复</td>
			 ...
			 <t:pageBarFrontThrid></t:pageBarFrontThrid>	
			</div>
			<!-- /.modal-content -->
		</div>
		<!-- /.modal-dialog -->
     ...
     var style1 =   new ol.style.Style({
  	...
  	var clearUndefineTable = function(){	 
		...
		nowPageNew = 1;
 		pageSizeNew = 2;
 		var jxsName = '';
      loadThridTable = function(){
     		var areaid = '${param.ADCODE99}';

  		$.ajax({  
              url: "${contextPath}/TSysSy/listJxxlMap",  
              data:{page:nowPageNew, rows:pageSizeNew,areaid:areaid,jxsName:jxsName},
              dataType: "json",  
              beforeSend:function(){
  		    	   //$('#loading').modal('show');
  		   	  },
           success: function (datas) {  
  	            ...
  	            var html = '';
  	        if(ajaxobj.length >0) {
  	        	initPageMsgNew(datas.total);
            	       html += ' <table class="table table-striped table-bordered table-hover clearNull "   >';
       	    	   html+='	<thead>'
          	  		  + '<tr>' + '<th style="font-size:14px;margin-left:3px;">序号<span style="font-size:10px;margin-left:3px;"></span></th>'
          	  	   	  + '<th style="font-size:14px;margin-left:3px;cursor:pointer" id="rzcpsl">检修线路名称</th>'
          	          + '<th style="font-size:14px;margin-left:3px;cursor:pointer" id="rzqysl">状态</th>'
          	  ...
          	  + '<th style="font-size:14px;margin-left:3px;cursor:pointer" id="rzqysl">装接容量（kVA）</th>'
          	          ...
          	          + '</thead>'
          	          + '<tbody>';
          	          var count=(nowPageNew - 1)*pageSizeNew;
            	    for(var i=0; i <ajaxobj.length;i++){
            	    	console.log(ajaxobj[i]);
            	        count++;
            	    	html+='<tr style=\'cursor:pointer\'>'  + '<td align="center">'+count+'</td>'  
            	    		+ '<td align="center">'+ajaxobj[i].MC+'</td>'
            	    		+ '<td align="center">'+ajaxobj[i].STATUS+'</td>'
