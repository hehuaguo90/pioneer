...
//以下样式针对表头的单元格.
TableSorter.prototype.NormalCss = "";//没有执行排序时的样式.
TableSorter.prototype.SortAscCss = "glyphicon glyphicon-arrow-up";//升序排序时的样式.
TableSorter.prototype.SortDescCss = "glyphicon glyphicon-arrow-down";//降序排序时的样式.
//初始化table的信息和操作.
...
this.ViewState[col] = false;
 if(col == 0) col = 1;
var obj = this.Table.rows[0].cells[col];
$(obj).parent().find('span').attr('class',this.NormalCss);
$(obj).find('span').attr('class',this.SortDescCss);
}
else
{
...
var obj = this.Table.rows[0].cells[col];
$(obj).parent().find('span').attr('class',this.NormalCss);
$(obj).find('span').attr('class',this.SortAscCss);
}
//if(col == 1) col = 0;
var Rank = [];
for(var x = 0; x < Sorter.length; x++)
