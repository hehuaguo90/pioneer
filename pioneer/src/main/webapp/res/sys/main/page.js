...
(function() {
	...
	var ALERT_TYPES = [ "log", "ok", "warning", "error", "info" ];
	...
	page.opener = {
		 ...
		 * @see main.post
		 * @see page.onRefresh
		 * @param pid 父级窗口的id，初始窗口一般为0，后续每打开一个窗口则递增+1;
		 *            [data] - 可附带的数据对象。
		 * @returns {Promise}
		 */
		refresh : function(data, pid) {
			if(!pid && pid != 0){
				pid = info.pid;
			}
			return exec("post", {
	...
	function selections(arr, param) {
		...
		if (kernel.lang.isNumber(param.size)) {
			if (len === 0)
				//errMsg = "请至少选择 " + param.size + " 条数据";
				errMsg = "请至少选择" + "一" + "条数据";
			else if (len !== param.size)
				//errMsg = "当前操作只能选择 " + param.size + " 条数据";
				errMsg = "当前操作只能选择" + "一" + "条数据";
		} else {
			if (kernel.lang.isNumber(param.min) && len < param.min)
				//errMsg = "请至少选择 " + param.min + " 条数据";
				errMsg = "请至少选择" + "一" + "条数据";

			if (kernel.lang.isNumber(param.max) && len > param.max)
				//errMsg = "当前操作最多选择 " + param.max + " 条数据";
				errMsg = "当前操作只能选择" + "一" + "条数据";
		}
	...
	function deleteData(arr, param) {
		var len = arr.length;
		if (len > 0) {
			page.confirm("是否确定删除"+len+"条数据？").then(function(boo) {
				if (boo) {
					var method = "GET";
				...
				onComplete : function(file, response) {
					if (response.status === 1) {
							...
							for(var j in picTypes){
								if(response.images[0].substring(response.images[0].indexOf(".")).toLowerCase().indexOf(picTypes[j])==-1){
									if(j == (picTypes.length-1)){
										page.error("请上传图片后缀名为"+dataOpts.picType+"的图片！");
										upload.loadEnd();
										return;
									}
								}else{
						...
						if(upload.imgURLs.length > dataOpts.maxPic){
							page.error("本处最多只能上传"+dataOpts.maxPic+"张图片！");
							page.send({
								url : page.upload.getDeleteURL(),
								data : {
